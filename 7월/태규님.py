...
def is_validation(company, blood_group, mail, name, website):
    try:
        is_result = True
        False_list = []
        ...
        if company in black_list:
         raise ValueError
        if blood_group not in blood_types:
            is_result = False
    ...
    # 각 조건에 따른 리턴
        if (is_result == False) and (len(False_list) == 1):
           return is_result, False_list[0]
        elif is_result == False:
            return is_result, False_list
        else:
...
def create_user():
    ...
    for data in user_data:
        ...
        # 유효성 검사
        is_result, False_list = is_validation(company, blood_group, mail, name, website)

        # 새로운 유저 리스트 만들기
        if is_result == True:
            user_list.append(data)

        # 잘못된 정보 수정 및 잘못된 정보 수 추출
        elif is_result == False:
            new_data = dict(data)
            if 'blood_group' in False_list:
                new_data['blood_group'] = None
            if 'company' in False_list: 
                new_data['company'] = None
            if 'mail' in False_list:
                new_data['mail'] = None
            if 'name' in False_list:
                new_data['name'] = None
            if 'website' in False_list:
                new_data['website'] = None
            user_list.append(new_data)
            wrong_case += 1
        else:
            wrong_case += 1
    print(f'잘못된 데이터로 구성된 유저의 수는 {wrong_case} 입니다.')
