...
#     stations.sort()  # 충전기가 설치된 정류장을 오름차순으로 정렬
#     current_position = 0  # 현재 위치 초기화
#     charging_count = 0  # 충전 횟수 초기화

#     for i in range(M):  # 충전기가 설치된 정류장을 순회
#         distance_to_next_station = stations[i] - current_position

#         # 현재 위치에서 다음 충전기 정류장까지의 거리가 한번 충전으로 이동 가능한 거리 이하라면 이동
#         if distance_to_next_station <= K:
#             current_position = stations[i]
#             continue

#         # 현재 위치에서 다음 충전기 정류장까지의 거리가 한번 충전으로 이동 가능한 거리보다 크다면,
#         # 충전 필요 횟수를 늘리고, 다음 충전기 정류장으로 이동하여 충전
#         else:
#             charging_count += 1
#             current_position += K
...
#     # 마지막 충전 이후에도 종점까지 도달하지 못했다면 종점에 도착할 수 없는 경우이므로 0을 반환
#     if current_position < N:
#         charging_count = 0
...
# for tc in range(1, T+1):
#     # 한번 충전으로 이동 가능한 정류장 수, 종점 정류장 번호, 충전기 설치된 정류장 수 입력
#     K, N, M = map(int, input().split())
#     stations = list(map(int, input().split()))  # 충전기 설치된 정류장 번호들 입력
